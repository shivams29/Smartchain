...
describe('Block',()=>{
    ...
    describe('adustDifficulty()',()=>{
        ...
        it('decreases the difficulty for a slowly mined block',()=>{
            expect(
                Block.adjustDifficulty({
                    lastBlock:{ blockHeaders : { difficulty: 6 , timestamp:1000 }},
                    timestamp:20000
                })
            ).toEqual(5)
        })
    ...
    describe('validateBlock()',()=>{
        ...
        it('rejects when the parent hash is inavlid',()=>{
            ...
            .rejects.toMatchObject({message:"The parent hash must be a hash of the last block's header"});
        })

        it('rejects when the number is inavlid',()=>{
            block.blockHeaders.number = 250
            expect(Block.validateBlock({ lastBlock,block }))
            .rejects.toMatchObject({message:"The block must increment the number by 1"});
        })

        it('rejects when the difficulty is inavlid',()=>{
            block.blockHeaders.difficulty = 250
            expect(Block.validateBlock({ lastBlock,block }))
            .rejects.toMatchObject({message:"The difficulty must only adjust by 1"});
        })
        ...
        it('rejects when the proof of work requirement is not met',()=>{
            ...
            expect(Block.validateBlock({ lastBlock,block }))
            .rejects.toMatchObject({message:"The block does not meet the proof of work requirement"});
